...
export function initDebtManagementModule(container) {
  if (!container) return;
  ...
  container.innerHTML = `
    <div class="bg-white p-3.5 sm:p-6 rounded-2xl sm:rounded-3xl border border-slate-200/70 shadow-card space-y-4 sm:space-y-6">
      ...
      <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <h2 class="text-base sm:text-xl font-black text-[#0B2C4D] tracking-tight leading-snug">Quản lý & Thu hồi công nợ (Debt Management)</h2>
        ...
        <div class="flex items-center gap-2">
          <button type="button" id="bulk-remind-btn" class="px-4 py-2 bg-amber-50 hover:bg-amber-100 border border-amber-200 rounded-2xl text-xs font-bold text-amber-700 transition flex items-center gap-2">
            <svg class="w-4 h-4 text-amber-600 fill-current" viewBox="0 0 24 24"><path d="M20 2H4c-1.1 0-1.99.9-1.99 2L2 22l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-7 12h-2v-2h2v2zm0-4h-2V6h2v4z"/></svg>
            <span>Nhắc nợ hàng loạt</span>
          </button>
  ...
  const bulkRemindBtn = container.querySelector('#bulk-remind-btn');
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.error('[DebtManagementModule] loadData error:', err);
  ...
  if (bulkRemindBtn) {
    bulkRemindBtn.onclick = () => {
      toast.show('Đã kích hoạt hàng đợi gửi thông báo nhắc nợ hàng loạt (Bulk Notification Dispatcher)...', 'success');
    };
  }
...
// Keep older alias for compatibility with test imports
export { initDebtManagementModule as initDebtReportModule };
